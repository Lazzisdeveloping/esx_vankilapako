...
AddEventHandler('esx_vankilapako:tooFar', function(currentStore)
...
AddEventHandler('esx_vankilapako:robberyStarted', function(currentStore)
	local _source  = source
	...
	if Stores[currentStore] then
		...
		if (os.time() - store.lastRobbed) < Config.eijaksaoottaa and store.lastRobbed ~= 0 then
			TriggerClientEvent('esx:showNotification', _source, _U('recently_hacked', Config.eijaksaoottaa - (os.time() - store.lastRobbed)))
			return
		end
		...
		if not rob then
			if cops >= Config.boliisia then
				...
				for i=1, #xPlayers, 1 do
					local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
					if xPlayer.job.name == 'police' or xPlayer.job.name == 'sheriff' then
						TriggerClientEvent('esx_vankilapako:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('esx_vankilapako:currentlyRobbing', _source, currentStore)
